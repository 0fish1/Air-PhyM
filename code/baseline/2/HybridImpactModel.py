...
class HybridImpactModel(nn.Module):
    def __init__(self, input_dim):
        ...
        self.target_feature_predictor = nn.Sequential(
            # nn.Linear(4, 32),  # 输入: [lat, lon, wind_speed, wind_dir]
            nn.Linear(5,32),
            nn.ReLU(),
            nn.LayerNorm(32),
    ...
    def forward(self, target_geo, neighbor_geo, neighbor_history, wind_data, weather_data):
        # 物理权重计算
        # dist = torch.cdist(target_geo.unsqueeze(0), neighbor_geo).squeeze(0)  # 距离
        # # dist = self.latlon_to_km(target_geo, neighbor_geo)
        # theta = self.calc_wind_angle(target_geo, neighbor_geo, wind_data[1])  # 风向夹角
        # A_phys = (1 / (dist ** self.alpha)) * (1 + torch.cos(theta)) ** self.beta
        dist = self.latlon_to_km(target_geo, neighbor_geo)
...
# # 示例输入
# target_geo = torch.tensor([39.9170, 116.3000])  # 经纬度
...
#                             [ 40.2170, 116.2300],
#                             [ 39.9820, 116.3970],
#                             [ 39.9140, 116.1840]])  # 周边站点经纬度
# neighbor_history = pollution_seq   # 周边站点24小时历史数据（6维特征）
# wind_data = torch.tensor([wind_speed.item(), wind_dir.item()])       # 风速3m/s, 风向300°（弧度制）
# # wind_data = torch.stack([wind_speed, wind_dir])

# model = HybridImpactModel(input_dim=6)
# impact_scores,phy_scores,data_scores = model(target_geo, neighbor_geo, neighbor_history, wind_data)
# print("混合影响得分:", impact_scores)
# print(phy_scores,"---")
# print(data_scores,"----")