...
class GCRNN(nn.Module):
    def __init__(self, num_nodes, in_features, hidden_dim, out_dim, adj, rnn_type="LSTM"):
...
if __name__ == "__main__":
    ...
    model = GCRNN(num_nodes=N, in_features=6, hidden_dim=64, out_dim=1, adj=adj).to(device)

    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)

    # 早停参数设置
    patience = 30  # 允许的验证集loss不下降的epoch数
    ...
    for epoch in range(max_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            counter = 0  # 重置计数器
            print(f"New best model found at epoch {epoch+1}, val_loss={val_loss:.4f}")
        else:
            counter += 1
            print(f"Val loss did not improve for {counter}/{patience} epochs")
    ...
    print("Loaded best model parameters for testing")
    ...
    }, 'best_gcrnn_model.pth')
    print("Best model saved to 'best_gcrnn_model.pth'")